    ...
    (function (i) {
      setTimeout(
        function () {
          ...
          else if (i == 3 && task.data.treasureBox.doneFlag == 0) qqreadbox();
          else if (i == 4 && task.data.taskList[0].doneFlag == 0) qqreadtake();
          else if (i == 5 && task.data.taskList[1].doneFlag == 0)
            qqreaddayread();
          else if (i == 6 && task.data.taskList[3].doneFlag == 0) qqreadvideo();
          else if (i == 7) qqreadsign();
          else if (i == 8) qqreadconfig();
          else if (i == 9 && sign.data.videoDoneFlag == 0) qqreadsign2();
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(toqqreadconfigurl, (error, response, data) => {
      ...
      if (task.data.taskList[2].doneFlag == 0) {
        for (
          let i = 0;
          i < config.data.pageParams.readTimeRewardTask.length;
          i++
        ) {
          setTimeout(() => {
        ...
        for (let i = 0; i < config.data.pageParams.readTimeTask.length; i++) {
          setTimeout(() => {
            ...
            const toqqreadssrurl = {
              url: `https://mqqapi.reader.qq.com/mqq/red_packet/user/read_time?seconds=${ssrid}`,
