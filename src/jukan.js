  ...
  !(async () => {
    if ($.isNode()) {
      ...
      if (process.env.JKD_COOKIE && process.env.JKD_COOKIE.indexOf("@") > -1) {
        ...
        console.log(
          `您的JKD_COOKIE选择的是用@隔开，共计 ${JKCookie.length} 个Cookie\n`
        );
      } else if (
        process.env.JKD_COOKIE &&
        process.env.JKD_COOKIE.indexOf("&") > -1
      ) {
        JKCookie = process.env.JKD_COOKIE.split("&");
        console.log(
          `您的JKD_COOKIE选择的是用&隔开，共计 ${JKCookie.length} 个Cookie\n`
        ...
        process.env.JKD_COOKIE &&
        process.env.JKD_COOKIE.indexOf("\n") > -1
      ) {
        JKCookie = process.env.JKD_COOKIE.split("\n");
        console.log(
          `您的JKD_COOKIE选择的是用换行符隔开，共计 ${JKCookie.length} 个Cookie\n`
        );
      ...
      if (process.env.JKD_DEBUG && process.env.JKD_DEBUG === "false")
        console.log = () => {};
    } else {
...
function bindTeacher() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl(
        "jkd/weixin20/member/bindTeacher.action",
        "teacherCode=24224873"
      ),
      async (err, resp, data) => {
        try {
          if (err) {
...
function getStageState() {
  return new Promise((resolve) => {
    $.post(
      taskGetUrl("jkd/weixin20/newactivity/readStageReward.action"),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function getTaskList() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskGetUrl(
        "jkd/mobile/base/welfaretask/indexList.action",
...
function doTask(taskId, taskName, action) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        `jkd/mobile/base/welfaretask/${action}.action`,
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function getUserInfo() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/newMobileMenu/infoMe.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === "ok") {
                ...
                $.gold =
                  ...
                  ": " +
                  data.userinfo.infoMeGoldItem.value;
...
function getTaskBoxProfit(boxType = 1) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jkd/task/getTaskBoxProfit.action", body),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function signShare(position) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/account/signShareAccount.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function adv(position) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/newmobile/stimulateAdv.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function rewardAdv(body) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/account/stimulateAdvAccount.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function openTimeBox() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/account/openTimeBoxAccount.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function getArticle(artId) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/newmobile/articleDetail.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function getVideo(artId) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/newmobile/artDetail.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function getStageReward(stage) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/weixin20/newactivity/getStageReward.action",
        `stage=${stage}`
      ),
      async (err, resp, data) => {
        try {
...
function call2(uuid, opttype = "ART_READ") {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/minfo/call.action",
        `jdata=${escape(
          JSON.stringify(body)
        )}&opttype=${opttype}&optversion=1.0`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === "ok") {
                if (opttype === "ART_READ") {
                  ...
                  $.log(
                    `文章剩余观看次数：${$.artcount}，视频剩余观看次数：${$.videocount}`
                  );
                } else {
...
function openArticle(artId) {
  ...
  let config = {
    url:
      "https://www.jukandiannews.com/jkd/weixin20/station/articleOpen.action",
    body: body,
    Host: "www.jukandiannews.com",
...
function video(artId) {
  ...
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("jkd/weixin20/station/cnzzinVideo.action", body),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function readAccount(artId, payType = 1) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/account/readAccount.action",
        `jsondata=${escape(JSON.stringify(body))}`
      ),
      async (err, resp, data) => {
        try {
...
function luckyDraw() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jkd/activity/advluckdraw/getLuckDrawGold.action"),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function luckyProfit() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jkd/activity/advluckdraw/getTotalLuckProfit.action"),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function getLuckyLevel() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jkd/activity/advluckdraw/getLuckDrawLevel.action"),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function getLuckyDrawBox(i) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jkd/activity/advluckdraw/getLuckDrawBox.action", body),
      async (err, resp, data) => {
        try {
          if (err) {
            $.log(`${JSON.stringify(err)}`);
...
function withDraw() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        "jkd/weixin20/userWithdraw/userWithdrawPost.action",
        `type=wx&sum=${sum}&mobile=&pid=0`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
