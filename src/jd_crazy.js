...
class CrazyJoy {
  ...
  gameState() {
    return new Promise((resolve) => {
      ...
      $.get(
        this.taskUrl("crazyJoy_user_gameState", body),
        async (err, resp, data) => {
          try {
            if (err) {
  ...
  getTaskState() {
    ...
    return new Promise((resolve) => {
      $.get(
        this.taskUrl("crazyJoy_task_getTaskState", body),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
  ...
  trade(joyLevel) {
    ...
    if (this.ctx.totalCoinAmount < cost) {
      console.log(
        `买不起等级为${joyLevel}的joy，需要${cost}，当前${this.ctx.totalCoinAmount}，跳过`
      );
      return;
    }
    console.log(
      `购买${joyLevel} ${this._max_level} ${this._max_level < joyLevel}`
    );
    const body = { action: "BUY", joyId: joyLevel, boxId: "" };
    return new Promise((resolve) => {
      $.get(
        this.taskUrl("crazyJoy_joy_trade", body),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
  ...
  moveOrMerge(from, to) {
    ...
    return new Promise((resolve) => {
      $.get(
        this.taskUrl("crazyJoy_joy_moveOrMerge", body),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
  ...
  shop() {
    ...
    return new Promise((resolve) => {
      $.get(
        this.taskUrl("crazyJoy_joy_allowBoughtList", body),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
