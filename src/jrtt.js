...
const $ = Env(jsname);
var tz = "";
var farmurl = process.env.farmurl;
var farmkey = process.env.farmkey;

var signurl = process.env.signurl;
var signkey = process.env.signkey;

var readurl = process.env.readurl;
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(profiturl, (error, response, data) => {
      ...
      if (result.err_no == 0) {
        other +=
          "🎉金币收益:" +
          result.data.score.amount +
          "\n🎉估计兑换现金:" +
          (result.data.score.amount / 30000).toFixed(2) +
          "\n🎉" +
          "现金收益:" +
          result.data.cash.amount +
          "\n";
  ...
  return new Promise((resolve, reject) => {
    ...
    $.post(readurl, (error, response, data) => {
      ...
      if (result.err_no == 0) {
        ...
        other +=
          "阅读进度" +
          result.data.icon_data.done_times +
          "/" +
          result.data.icon_data.read_limit +
          "\n";
      }
      if (result.err_no == 4) {
        other += "📣文章阅读\n";
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(sleepstatusurl, (error, response, data) => {
      ...
      if (result.data.sleeping == false) {
        ...
        other +=
          "当前状态:酣睡中,已睡" +
          parseInt(result.data.sleep_last_time / 3600) +
          "小时" +
          parseInt((result.data.sleep_last_time % 3600) / 60) +
          "分钟" +
          parseInt((result.data.sleep_last_time % 3600) % 60) +
          "秒\n";
        ...
        if (
          result.data.sleep_unexchanged_score == 3600 ||
          parseInt(result.data.sleep_last_time / 3600) == 12
        ) {
