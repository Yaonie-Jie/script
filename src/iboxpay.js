...
const $ = Env("笑谱");
$.idx =
  ($.idx = ($.getval("iboxpaySuffix") || "1") - 1) > 0 ? $.idx + 1 + "" : ""; // 账号扩展字符
...
($.message = ""), (COOKIES_SPLIT = ""), (CASH = ""), (LIVE = 60);
let ins = 0,
  livecs = 0;
...
function GetCookie() {
  ...
  if (
    $request &&
    $request.url.indexOf("nf_user_center_web") >= 0 &&
    $request.url.indexOf("get_context_info") >= 0
  ) {
    ...
    $.log(
      `[${
        $.name + $.idx
      }] 获取header✅: 成功,iboxpayheaderVal: ${iboxpayheaderVal}`
    );
...
function msgShow() {
  return new Promise(async (resolve) => {
    ...
    if (
      notifyInterval == 3 &&
      (nowTimes.getHours() === 6 ||
        nowTimes.getHours() === 12 ||
        nowTimes.getHours() === 18 ||
    ...
    setTimeout(() => {
      ...
      $.get(url, async (err, resp, data) => {
        try {
          ...
          $.message +=
            "【金币信息】：今日金币" +
            $.goldcoin.data.coinSum +
            ",预估金额" +
            $.goldcoin.data.balanceSum / 100 +
            "元" +
            "\n";
        } catch (e) {
          $.logErr(e, resp);
    ...
    setTimeout(() => {
      ...
      $.get(url, async (err, resp, data) => {
        try {
          ...
          $.message +=
            "【账户信息】：可提余额" +
            $.coin.data.balance / 100 +
            ",明日入账" +
            $.coin.data.tomorrowAmt / 100 +
            "元" +
            "\n";
        } catch (e) {
          $.logErr(e, resp);
    ...
    setTimeout(() => {
      for (let i = 0; i < CS; i++) {
        setTimeout(() => {
          ...
          videoPublishId = playbodyVal.substring(
            playbodyVal.indexOf("videoPublishId") + 17,
            playbodyVal.indexOf(`","pl`)
          );
          if (i == 2) {
            videoPublishId3 = playbodyVal.substring(
              playbodyVal.indexOf("videoPublishId") + 17,
              playbodyVal.indexOf(`","pl`)
            );
          }
          if (i == 3) {
            videoPublishId4 = playbodyVal.substring(
              playbodyVal.indexOf("videoPublishId") + 17,
              playbodyVal.indexOf(`","pl`)
            );
          }
          if (i == 4) {
            videoPublishId5 = playbodyVal.substring(
              playbodyVal.indexOf("videoPublishId") + 17,
              playbodyVal.indexOf(`","pl`)
            );
          }
          if (i == 5) {
            videoPublishId6 = playbodyVal.substring(
              playbodyVal.indexOf("videoPublishId") + 17,
              playbodyVal.indexOf(`","pl`)
            );
          }
...
function goldvideo(timeout = 60000) {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      header = iboxpayheaderVal.replace(`${oldtime}`, `${tts}`);
      goldvideobodyVal = `{"type":2,"videoList":[{"videoId":"${videoPublishId3}","type":1,"isFinishWatch":false},{"videoId":"${videoPublishId4}","type":1,"isFinishWatch":false},{"videoId":"${videoPublishId5}","type":1,"isFinishWatch":false},{"videoId":"${videoPublishId6}","type":1,"isFinishWatch":false}],"actId":"302"}`;
      let url = {
        url: `https://veishop.iboxpay.com/nf_gateway/nf_customer_activity/day_cash/v1/give_gold_coin_by_video.json`,
        headers: JSON.parse(header),
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if ($.goldvideo.resultCode == 1) {
            ...
            $.message +=
              "【金蛋视频奖励】：获得" +
              $.goldvideo.data.goldCoinNumber +
              "金币\n";
          }
          if ($.goldvideo.resultCode == 0) {
            console.log($.goldvideo.errorDesc + "\n");
            $.message += "【金蛋视频奖励】：" + $.goldvideo.errorDesc + "\n";
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
    ...
    setTimeout(() => {
      ...
      $.get(url, async (err, resp, data) => {
        try {
          ...
          if (
            $.sylist.resultCode == 1 &&
            data.split("obtainDate")[1].match(/500/g)
          ) {
            live = data.split("obtainDate")[1].match(/500/g);
            livecs = live.length;
            console.log(
              "已获得直播奖励 " + livecs + " 次，共" + livecs * 500 + "金币\n"
            );
            $.message +=
              "【直播收益】：已获得直播奖励 " +
              livecs +
...
function goldcointowallet(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout(() => {
      let body = `auth=${pcpopclub}&userid=${app_userid}&cashtype=3&goldcoin_amount=${
        CASH * 10000
      }&validatecode=&faceno=&a=18&pm=1&v=1.7.0&device_id=${app_deviceid}&sessionid=${sessionid}&_timestamp=${tts}`;
      let url = {
