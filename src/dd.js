...
if ($.isRequest) {
  ...
  !(async () => {
    $.Ticket =
      "T7GCriuJfs3gJJfx987wF7VdG64emtc9S2yjjiB27EskzDuOwlAQRNG93Lhldb2_ezfz8QwkDwlEZLF3ZBxVUvfsTCfIiy-OMUXImImQu7sxM6Fe19RLyZ5L6sYshJpqbjX3ZsxKgPF1zjeRRiu9qrexyiXjl1A2NmLncXvefzbCX8bfIXmvY_1I_wRHpVFHkTAup3k97u8AAAD__w%3D%3D";
    $.city = "5";
    ...
    if (!$.Ticket || !$.city) {
      ...
      if (
        $.now >= EIGHT_O_CLOCK_PM - 10 * 1000 &&
        $.now <= EIGHT_O_CLOCK_PM + 60 * 1000
      ) {
    ...
    .then((resp) => {
      if (resp.statusCode == 403) {
        ...
        if (obj && obj.errno == 0) {
          ...
          $.detail +=
            "账户共有 " + total + " 福利金，可抵扣 " + total / 100 + " 元。";
          if (obj.data.message && obj.data.message.text)
            $.info(obj.data.message.text);
        ...
        for (let a of obj.data.calendar[today]) {
          if (a.act_conf.receive_start_at) {
            if (a.act_conf.receive_start_at.match("09:00:00")) {
              ...
              $.info(
                a.act_conf.receive_start_at + ": " + a.act_id + " 已存 ✅"
              );
              ...
              $.info(
                a.act_conf.receive_start_at + ": " + a.act_id + " 未存 ❌"
              );
              //$.couponids += "\n券编号：" + a.act_conf.receive_start_at + ": " + a.act_id;
              //$.couponids += " 未存 ❌";
            }
  ...
  return $.post({
    ...
    .then((resp) => {
      ...
      for (let i = 0; i < list.length; i++) {
        if (
          list[i].timesegs[0].end_time - list[i].timesegs[0].start_time ==
          604799
        )
          $.pointSignURL = list[i].link;
      }
      $.realPointSignURL = $.pointSignURL
        ? $.pointSignURL
        : $.sparePointSignURL;
    })
    .catch((err) => {
...
function pointInfo() {
  return $.get({
    url:
      pointURL +
      "/user/account?source_id=ckjf_10001&token=" +
      encodeURIComponent($.Ticket),
  })
    .then((resp) => {
...
async function reward() {
  ...
  if ($.rewardtotal)
    $.detail += "捡回遗忘的 " + $.rewardtotal.toFixed(2) + " 元福利金。";
}
...
function joinInstance() {
  ...
  return $.post({
    url:
      mainURL +
      "/toggle/api/instance/join?ticket=" +
      encodeURIComponent($.Ticket),
    body:
      "scene=" + $.instanceScene + "&activity_instance_id=" + $.instancechoose,
  })
    .then((resp) => {
...
function grabCoupons(id) {
  return $.post({
    ...
    body:
      '{"app_id":"common","city_id":"' + $.city + '","act_id":"' + id + '"}',
  })
    .then((resp) => {
      $.log(
      ...
      if (obj.errno == 0) {
        ...
        throw new ERR.BodyError(
          "请求体错误，请开启抓包运行脚本后反馈抓包内容。"
        );
      } else {
        $.error(resp.body);
        throw new ERR.BodyError(
          id +
            ": 抢券返回错误，请在 BoxJs 中开启调试模式运行后反馈日志。\n" +
            obj.errmsg
        );
      }
    })
