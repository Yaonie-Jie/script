...
function getIsvToken() {
  return new Promise((resolve) => {
    $.post(
      jdUrl("encrypt/pin?appId=dafbe42d5bff9d82298e5230eb8c3f79"),
      async (err, resp, data) => {
        try {
          if (err) {
...
function getIsvToken2() {
  return new Promise((resolve) => {
    $.post(
      jdUrl(
        "user/token?appId=dafbe42d5bff9d82298e5230eb8c3f79&client=m&url=pengyougou.m.jd.com"
      ),
      async (err, resp, data) => {
        try {
          if (err) {
...
function getActInfo(inviter = null) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("platform/active/role/login", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function checkLogin() {
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/logincheck", {
...
function getTaskList() {
  return new Promise((resolve) => {
    $.post(
      taskUrl("platform/active/jingdong/gametasks", {
        activeid: "A_112790_R_1_D_20201028",
        id: $.id,
        token: $.gameToken,
        authcode: $.authcode,
            ...
            if (safeGet(data)) {
              ...
              for (let task of data.tasks) {
                if (task.res.sName === "闯关集星") {
                  ...
                  for (
                    let i = task.state.iFreshTimes;
                    i < task.res.iFreshTimes;
                    ++i
                  ) {
...
function beginLevel() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/beginLevel", obj2param(body), true),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function endLevel() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/endLevel", obj2param(body), true),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
            ...
            if (safeGet(data)) {
              ...
              // console.log(data)
              if (data.code === 0) {
                const level = data.allLevels.filter(
                  (vo) => parseInt(vo.id) === $.level
                );
                if (level.length > 0) {
                  console.log(
                    `第${$.level++}关已通关，上报${level[0].maxScore}分，获得${
                      level[0].maxStar
                    }星星`
                  );
                } else {
                  console.log(
                    `第${$.level}关分数上报失败，错误信息:${JSON.stringify(
                      data
                    )}`
                  );
                }
                ...
                console.log(
                  `第${$.level}关分数上报失败，错误信息:${JSON.stringify(data)}`
                );
              }
            }
          }
...
function uploadTask(taskType, value) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("platform//role/base/uploadtask", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function finishTask(taskId) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("/platform/active/jingdong/finishtask", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function execute(body) {
  return new Promise((resolve) => {
    $.post(
      taskUrl("/platform/active/jingdong/execute", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function getDailyMatch() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/getDailyMatch", obj2param(body), true),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                // console.log(data)
                $.maxScore = parseInt(
                  data.dailyMatchList[data.dailyMatchList.length - 1]["sScore"]
                );
                if (
                  data.dayInfo.score >= $.maxScore &&
                  data.dayInfo.boxAwardIndex < 2
                ) {
...
function beginDailyMatch() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/beginDailyMatch", obj2param(body), true),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                console.log(
                  `每日挑战开启成功，本日挑战次数${data.dayInfo.dayPlayNums}/2`
                );
                ...
                console.log(
                  `每日挑战开启失败，错误信息：${JSON.stringify(data)}`
                );
              }
            }
          }
...
function endDailyMatch() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("eliminate_jd/game/local/endDailyMatch", obj2param(body), true),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                ...
                console.log(
                  `每日挑战完成失败，错误信息：${JSON.stringify(data)}`
                );
              }
            }
          }
...
function getDailyMatchAward() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl(
        "eliminate_jd/game/local/getDailyMatchAward",
        obj2param(body),
        true
      ),
      async (err, resp, data) => {
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                console.log(
                  `每日挑战领取成功，获得${
                    data.reward[0] === "11001" ? "消消乐星星" : "未知道具"
                  }*${data.reward[1]}`
                );
              } else {
                console.log(
                  `每日挑战领取失败，错误信息：${JSON.stringify(data)}`
                );
              }
            }
          }
...
function marketGoods() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("/platform/active/role/marketgoods", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
...
function buyGood(consumeid) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("/platform/active/role/marketbuy", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                console.log(
                  `商品兑换成功，获得${
                    data.item[0].itemid === "JD29" ? "京豆" : "未知奖品"
                  } * ${data.item[0].count}`
                );
              } else {
                console.log(`任务完成失败，错误信息：${JSON.stringify(data)}`);
...
function taskUrl(functionId, body = {}, decrypt = false) {
  return {
    ...
    headers: {
      ...
      "Content-Type": decrypt
        ? "application/x-www-form-urlencoded"
        : "application/json",
      Referer:
        "https://game-cdn.moxigame.cn/eliminateJD/index.html?activeId=A_112790_R_1_D_20201028",
      "User-Agent": $.isNode()
...
function getDailyReward() {
  let headers = {
    ...
    "user-agent":
      "Mozilla/5.0 (iPhone; CPU iPhone OS 14_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/7.0.20(0x1700142b) NetType/WIFI Language/zh_CN",
  ...
  let body = {
    ...
    type: 1,
    eid:
      "DPIFTWTK6N7EEVHNJW3JW7PZDALZNTODNUBBYWQBAYXPAJCH7AMIUEGY7LVCWCRILXXEYOAM5DXZJKY5Y5AZNHQFJI",
  };
  $.get(
    {
      url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${new Date().getTime()}&loginType=2&body=${escape(
        JSON.stringify(body)
